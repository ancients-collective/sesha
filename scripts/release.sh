...
#   2. Pre-flight checks (clean tree, on release/* branch)
...
#   5. Runs tests
#   6. Commits on release branch with conventional commit format
#   7. Merges to main, tags, pushes, and merges back to develop
#
# Branching model (Gitflow-lite):
#   develop  → release/x.y.z → main (tagged) → back-merge to develop
set -euo pipefail
...
if [[ "$BRANCH" != release/* ]]; then
  error "Not on a release branch (currently on '$BRANCH')."
  echo ""
  echo "  Create one with: make release-branch VERSION=${NEW_VERSION}"
  exit 1
fi
...
# Commit, tag, push
# ---------------------------------------------------------------------------
info "Committing on release branch"
...
git merge --no-ff "$BRANCH" -m "chore(release): merge ${BRANCH} for v${NEW_VERSION}"
git tag "v${NEW_VERSION}"

echo ""
info "Back-merging to develop"

git checkout develop
git pull origin develop
git merge --no-ff main -m "chore(release): back-merge v${NEW_VERSION} to develop"

echo ""
info "Pushing all branches and tag"
